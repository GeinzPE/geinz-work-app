...
async function downloadTelegramFile(fileId) {
  ...
  if (!filePath)
    throw new Error("No se pudo obtener el archivo desde Telegram.");
  const res = await fetch(`${TELEGRAM_FILE_API}/${filePath}`);
  if (!res.ok)
    throw new Error(
      `No se pudo descargar el archivo de Telegram (HTTP ${res.status}).`,
    );
...
function kbGenerate() {
  return {
    inline_keyboard: [
      [{ text: "🎨 Generar imagen", callback_data: "menu:generate" }],
    ],
  };
}
...
function kbCategories() {
  return {
    inline_keyboard: [
      [
        { text: "🧮 Matemática / Math", callback_data: "cat:math" },
        { text: "💻 Programación / Programming", callback_data: "cat:prog" },
      ],
    ],
  };
}
  ...
  return {
    inline_keyboard: [
      [
        ...
        { text: "◻️ 1:1 (Cuadrado / Square)", callback_data: "ratio:1-1" },
      ],
    ],
  };
}
...
function kbWatermark() {
  return {
    inline_keyboard: [
      [
        { text: "✅ Sí, usar logo / Yes, use logo", callback_data: "wm:yes" },
        { text: "🚫 Sin marca de agua / No watermark", callback_data: "wm:no" },
      ],
    ],
  };
}
...
function kbSaveCategory() {
  ...
  return {
    inline_keyboard: [
      [
        {
          text: "🗂️ Problemáticas / Problematics",
          callback_data: "savecat:problematics",
        },
        {
          text: "🌟 Motivación / Motivation",
          callback_data: "savecat:motivaciones",
        },
      ],
    ],
  };
...
async function runGeneration(
  chatId,
  { rubroKey, ratio, sourceCategory, useLogo },
) {
  await sendMessage(
    chatId,
    "⚙️ Generando tu trivia, un momento… / Generating your trivia, one moment…",
  );
  ...
  for (const lang of ["es", "en"]) {
    ...
    const hashtags = buildHashtags(lang, rubroKey);
    const caption = `<b>${trivia.titulo}</b>\n\n<code>${trivia.codigo}</code>\n\n${trivia.pregunta}\n\n${hashtags}`;
    await sendPhoto(chatId, outputUrl, caption);
  }

  await sendMessage(chatId, "¿Quieres otra? / Want another one?", {
    reply_markup: kbGenerate(),
  });
}
...
async function handlePostsQuery(chatId) {
  try {
    ...
    await sendMessage(
      chatId,
      "❌ Error al consultar tus publicaciones: " + err.message,
    );
  }
}
  ...
  try {
    if (session.state === "await_send_qty_source") {
      ...
      if (category === "ambas") {
        ...
        const b = await listCategoryImages("motivaciones", {
          limit: limit - a.items.length,
        });
      ...
      if (!items.length) {
        ...
        await sendMessage(
          chatId,
          `✅ Te envié ${items.length} publicación(es).`,
        );
      }
    }
  } catch (err) {
...
async function handleUpdate(update) {
  ...
  if (update.callback_query) {
    ...
    if (data === "menu:generate") {
      ...
      await sendMessage(chatId, "Elige una categoría / Choose a category:", {
        reply_markup: kbCategories(),
      });
      return;
    }

    if (data.startsWith("cat:")) {
      const [, rubroKey] = data.split(":");
      await setSession(chatId, { state: "await_ratio", rubroKey });
      await sendMessage(
        chatId,
        "¿Qué formato quieres? / What format do you want?",
        { reply_markup: kbRatio() },
      );
      return;
    }
    ...
    if (data.startsWith("ratio:")) {
      ...
      if (!session || session.state !== "await_ratio") {
        await sendMessage(
          chatId,
          "Sesión expirada, escribe algo para empezar de nuevo. / Session expired, send any message to start over.",
        );
        return;
      }
      await setSession(chatId, {
        state: "await_watermark",
        ratio,
        sourceCategory: "problematics",
      });
      await sendMessage(
        chatId,
        "¿Quieres usar el logo / marca de agua? / Do you want to use the logo / watermark?",
        { reply_markup: kbWatermark() },
      );
      return;
    }
    if (data.startsWith("src:")) {
      ...
      const q =
        session.lang === "en"
          ? "Do you want to use the logo / watermark?"
          : "¿Quieres usar el logo / marca de agua?";
    ...
    if (data === "save:yes") {
      ...
      if (!queue.length) {
        await sendMessage(
          chatId,
          "No encontré la imagen, mándala de nuevo por favor. / I couldn't find the image, please send it again.",
        );
        return;
      }
      ...
      await sendMessage(
        chatId,
        "¿En qué carpeta la guardo? / Which folder should I save it in?",
        { reply_markup: kbSaveCategory() },
      );
      return;
    }
    ...
    if (data.startsWith("savecat:")) {
      ...
      if (!queueBefore.length || session.state !== "await_save_category") {
        await sendMessage(
          chatId,
          "Sesión expirada, manda la foto de nuevo. / Session expired, send the photo again.",
        );
        return;
      }
      ...
      try {
        ...
        await sendMessage(
          chatId,
          `✅ Guardada en <code>${path}</code> (${kb} KB, WebP 9:16)`,
        );
      } catch (err) {
        console.error(err);
        await sendMessage(
    ...
    if (data === "sendimgs:yes") {
      ...
      await sendMessage(chatId, "¿De qué carpeta? / Which folder?", {
        reply_markup: kbSendCategory(),
      });
      return;
    }
    ...
    if (data.startsWith("sendqty:")) {
      ...
      if (
        !session ||
        (session.state !== "await_send_qty_source" &&
          session.state !== "await_send_qty_posts")
      ) {
        await sendMessage(
          chatId,
          "Sesión expirada, escribe algo para empezar de nuevo. / Session expired, send any message to start over.",
  ...
  if (update.message) {
    ...
    if (Array.isArray(update.message.photo) && update.message.photo.length) {
      ...
      const session = await getSession(chatId);
      const alreadyAsking =
        session?.state === "await_save_confirm" ||
        session?.state === "await_save_category";
    ...
    if (
      activeSession &&
      (activeSession.state === "await_send_qty_source" ||
        activeSession.state === "await_send_qty_posts") &&
      /^\d+$/.test(text)
    ) {
    ...
    if (isStatsQuery(text)) {
      ...
      await sendMessage(chatId, "📤 ¿Quieres que te las envíe?", {
        reply_markup: kbYesNo("sendimgs:yes", "sendimgs:no"),
      });
      return;
    }
    ...
    if (isPostsQuery(text)) {
      ...
      await sendMessage(chatId, "📤 ¿Quieres que te envíe algunas?", {
        reply_markup: kbYesNo("sendposts:yes", "sendposts:no"),
      });
      return;
    }
    ...
    await setSession(chatId, { state: "idle" });
    await sendMessage(chatId, "👋 ¡Hola! ¿Qué quieres hacer?", {
      reply_markup: kbGenerate(),
    });
  }
}
